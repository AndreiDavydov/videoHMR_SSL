...
def get_losses_by_config(cfg, device0):
    ...
    losses_weights = (
        edict()
    )  # weights of losses (in case they are computed in a weighted combination)
    ...
    for loss_type in cfg.LOSSES:  # "loss_type" must be a name of a loss
        ...
        if "DIR" in loss_dict:
            ...
            losses[loss_type] = eval(f"{loss_dict.DIR}" + "." + f"{loss_dict.NAME}")(
                **params
            )
        else:
            losses[loss_type] = None # dummy loss, need it here to initialize a metrics tracker

        losses_weights[loss_type] = (
            1.0 if "WEIGHT" not in loss_dict else loss_dict.WEIGHT
        )  # default is 1.
