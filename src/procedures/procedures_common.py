...
def one_epoch(train=TRAIN, valid=VALID, epoch_info=EPOCH_INFO):
    ...
    def do_one_epoch(trainer):
        ...
        if (
            trainer.cur_epoch == 1
            and "VALID_FIRST" in trainer.cfg
            and trainer.cfg.VALID_FIRST
        ):
            trainer.logger.info("=> Cold-run validation (only for epoch 0)")
            trainer.cur_epoch = 0
        ...
        if trainer.sched.do_scheduling:
            ...
            for model_name, sched in scheds.items():
                ...
                if new_lr is not None:
                    trainer.logger.info(
                        f'=> "{model_name}" learning rate reduced to {new_lr:.2e}.'
                    )

            ### finish the training in case all lrs got minimum value
            if sum(trainer.sched.sched_finished.values()) == len(
                trainer.sched.sched_finished
            ):
                trainer.logger.info(
...
def get_procedures_by_config(cfg):
    ...
    assert (
        cfg.PROCEDURE != ""
    ), "procedure name must be specified! Check the <exp>.yaml file"
    ...
    setup = eval("proc.setup") if "setup" in directory else SETUP
