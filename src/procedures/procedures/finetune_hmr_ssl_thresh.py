...
def train(trainer):
    ...
    ### loss thresholding
    thresh = trainer.cfg.THRESH  # if OFC loss value is bigger than thresh, than ignore or scale it
    thresh_val = thresh.thresh
    thresh_mode = thresh.mode # "ignore" or "scale"
    batches_thresholded = 0
    if thresh_mode == "scale":
        thresh_scale = thresh.scale
    ####
    ...
    for batch_idx, sample in enumerate(trainer.dataload.video_finetune, start=1):
        ...
        if trainer.losses_weights.flow_2d > 0:
            ...
            #####
            if flow_2d > thresh_val:
                batches_thresholded += 1
                ### loss should be penalized somehow
                if thresh_mode == "ignore":
                    continue
                elif thresh_mode == "scale":
                    flow_2d = flow_2d / thresh_scale
            #####
        ...
        if trainer.losses_weights.shape_smooth > 0:
            ...
            trainer.meters.train.shape_smooth.update_raw(shape_smooth.item())
        ...
        if trainer.losses_weights.j3d_smooth > 0:
            ...
            trainer.meters.train.j3d_smooth.update_raw(j3d_smooth.item())

        ### consistency
        if trainer.losses_weights.j3d_cons > 0:
            j3d_cons = ((j3d - j3d_best) ** 2).mean()
            loss += j3d_cons * trainer.losses_weights.j3d_cons
            trainer.meters.train.j3d_cons.update_raw(j3d_cons.item())
        ...
        if trainer.losses_weights.shape_cons > 0:
            ...
            trainer.meters.train.shape_cons.update_raw(shape_cons.item())
        ...
        trainer.meters.train.full.update_raw(loss.item())

        total_time = time() - absolute_start
        status_msg(trainer, batch_idx, dl_len, trainer.meters.train.full, total_time)
    
    trainer.logger.info(f"=> BATCHES THRESHOLDED: {batches_thresholded} / {dl_len}")
