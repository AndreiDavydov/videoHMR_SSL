...
class TemporalEncoder(nn.Module):
    def __init__(self, n_layers=1, seq_len=16, hidden_size=2048):
        ...
        self.attention = TemporalAttention(
            attention_size=2048, seq_len=3, non_linearity="tanh"
        )

    def forward(self, x, is_train=False):
        # NTF -> TNF
        y, state = self.gru_cur(
            x.permute(1, 0, 2)
        )  # y: Tx N x (num_dirs x hidden size)
