...
def init_exp_folder(cfg):
    ...
    ), "Specify the Output folder (where to save models, checkpoints, etc.)!"
...
def create_logger(output_dir, cfg):
    ...
    return logger, log_filename, str(final_log_file)
...
def copy_proc_file(trainer):
    if "FAKERUN" in trainer.cfg and trainer.cfg.FAKERUN:
        return
    proc_file = f"./src/procedures/procedures/{trainer.cfg.PROCEDURE}.py"
