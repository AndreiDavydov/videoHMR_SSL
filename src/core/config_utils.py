...
def get_config(exp_config):
    ...
    if isinstance(exp_config, str):
        ...
        raise ValueError(
            '"exp_config" input must be of type either PATH (to the yaml file) or EASYDICT!'
        )

    new_config = update_config(exp_config)
    new_config.CONFIG_FILENAME = (
        config_filename  # path to the config file relative to the launching point
    )
    return new_config
