...
from src.functional.image_augmentations import Augment
...
IMAGES_PATH = '/cvlabsrc1/cvlab/dataset_H36M_SMPL/Human36m/OpenPose/'

ANNOT_PATH = '/cvlabdata2/home/davydov/videoHMR_SSL/data/h36m/dict_h36m_desc.pkl'
with open(ANNOT_PATH, 'rb') as f:
    DICT_H36M_DESC = pickle.load(f)

KEYPOINTS_WORLD_PATH = '/cvlabdata2/home/davydov/videoHMR_SSL/data/h36m/dict_h36m_keypoints_world.pkl'
with open(KEYPOINTS_WORLD_PATH, 'rb') as f:
    DICT_H36M_KEYPOINTS_WORLD = pickle.load(f)

CAMERA_PARAMS_PATH = '/cvlabdata2/home/davydov/videoHMR_SSL/data/h36m/camera-parameters.json'
...
class H36M(torch.utils.data.Dataset):
    def __init__(self, image_transform=TRANSFORM_DEFAULT, do_crop=True,
                ...
                verbose=False, use_m=True, augmentations=None, image_size=IMAGE_SIZE
                    ):
    ...
    def __getitem__(self, idx):
        ...
        if self.do_crop:
            img, pose2d, _, crop_transform = _get_cropped_img_tight(img, pose2d, image_size=(self.image_size, self.image_size))
            # img, pose2d, _, crop_transform = _get_cropped_img(img, pose2d, image_size=(self.image_size, self.image_size))
        
        ### image augmentation: rotate, flip, colorjitter
        img, pose3d, pose2d = self.augment(img, pose3d, pose2d)
        pose_mask = _get_pose_mask(pose2d, self.image_size) # if pose_mask[idx], then joint is inside the image
        ...
        out = {}
        out['img'] = img
        out['pose2d'] = torch.from_numpy(pose2d)
        out['pose_mask'] = torch.from_numpy(pose_mask)
        out['pose3d'] = torch.from_numpy(pose3d)
        meta = {'subj':subj, 'scen':scen, 'seq':seq, 'cam':cam, 'frame':frame}
        out['meta'] = meta

        if 'pose3d_world' in self.out_types:
            out['pose3d_world'] = torch.from_numpy(pose3d_world)
        if 'intrinsics' in self.out_types:
            out['intrinsics'] = torch.from_numpy(intrinsics)
        if 'intrinsics_inv' in self.out_types:
            out['intrinsics_inv'] = torch.from_numpy(intrinsics_inv)
        if 'extrinsics' in self.out_types:
            out['extrinsics'] = torch.from_numpy(extrinsics)
        if 'pelvis3d' in self.out_types:
            out['pelvis3d'] = torch.from_numpy(pelvis3d)
        if 'crop_transform' in self.out_types:
            out['crop_transform'] = torch.from_numpy(crop_transform)
...
def _get_cropped_img(img, pose2d, pose2d_mask=None, image_size=(IMAGE_SIZE, IMAGE_SIZE)): 
    # resize in IMAGE_SIZE x IMAGE_SIZE automatically
    ...
    new_img, pose2d[pose2d_mask], pose_scale = _resize(new_img, pose2d[pose2d_mask], output_shape=image_size)
    pose_transform = np.stack((pose_shift, pose_scale))

    return new_img, pose2d, pose2d_mask, pose_transform


def do_pad(img): # up to square
    ...
    if delta == 0:
        ...
        img_new = np.zeros((width, width, 3), dtype=int)
        img_new[abs(delta)//2 : abs(delta)//2 + height] = img
        pad = ((abs(delta)//2, abs(delta) - abs(delta)//2), (0,0))
    else:
        img_new = np.zeros((height, height, 3))
        img_new[:, delta//2 : delta//2 + width] = img
        pad = ((0,0),(delta//2, delta - delta//2))
    return img_new, pad

def _get_cropped_img_tight(img, pose2d, pose2d_mask=None, image_size=(IMAGE_SIZE, IMAGE_SIZE), num_pixels_to_pad=20): 
    # resize in IMAGE_SIZE x IMAGE_SIZE automatically
    ...
    img_height, img_width = img.shape[:2]
    _2d_min_w, _2d_max_w = int(np.min(pose2d[pose2d_mask][:,0])), int(np.max(pose2d[pose2d_mask][:,0]))
    _2d_min_h, _2d_max_h = int(np.min(pose2d[pose2d_mask][:,1])), int(np.max(pose2d[pose2d_mask][:,1]))

    if _2d_min_w < 0: _2d_min_w = 0
    if _2d_min_h < 0: _2d_min_h = 0
    if _2d_max_w > img_width - 1: _2d_max_w = img_width - 1
    if _2d_max_h > img_height - 1: _2d_max_h = img_height - 1
    
    img_crop_tight = img[_2d_min_h:_2d_max_h, _2d_min_w:_2d_max_w]
    pose_shift = - np.array([_2d_min_w, _2d_min_h]).reshape(1, 2)

    img_crop_tight_pad = np.concatenate([np.pad(img_crop_tight[:,:,i], (num_pixels_to_pad, num_pixels_to_pad))[...,None]  for i in range(3)], axis=2)
    pose_shift += np.array([num_pixels_to_pad, num_pixels_to_pad]).reshape(1, 2)

    img_crop_tight_pad_square, pad = do_pad(img_crop_tight_pad)
    (top, bottom), (left, right) = pad
    pose_shift += np.array([left, top]).reshape(1, 2)

    pose2d += pose_shift
    pose2d[~pose2d_mask] = -1.
    new_img, pose2d[pose2d_mask], pose_scale = _resize(img_crop_tight_pad_square, pose2d[pose2d_mask], output_shape=image_size)
    pose_transform = np.stack((pose_shift, pose_scale))
