...
UNNORMALIZE = UnNormalize(mean=IMG_NORM_MEAN, std=IMG_NORM_STD)
NORMALIZE = transforms.Normalize(mean=IMG_NORM_MEAN, std=IMG_NORM_STD)
TRANSFORMS = {
...
def get_datasets_by_config(cfg):
    ...
    for dset in cfg.DATASETS:
        ...
        if "CONCAT" in dset_dict and dset_dict.CONCAT:
            assert (
                "DSETS" in dset_dict
            ), "If one dset is Concat of some other, the list of such must be specified."
            datasets[dset] = torch.utils.data.ConcatDataset(
                [datasets[ds] for ds in dset_dict.DSETS]
            )
        else:
            exec(f"from . import {dset_dict.DIR}")
            params = dset_dict.PARAMS if "PARAMS" in dset_dict else {}
            datasets[dset] = eval(f"{dset_dict.DIR}" + "." + f"{dset_dict.NAME}")(
                **params
            )
...
def get_dataloaders_by_config(cfg, datasets, num_gpus=1):
    ...
    for dset in cfg.DATALOAD:
        ...
        if dset == "multidl":  ### uses custom MultiDataLoaderForIterations class
            ...
            exec("from .dataloaders_custom import MultiDataLoaderForIterations")
            dls = [dataloaders[dl] for dl in dl_dict.dls]
            dl = eval("MultiDataLoaderForIterations")(
                dls, dl_dict.NUM_ITERATIONS_PER_EPOCH
            )
            dataloaders[dset] = dl
            continue
        ...
        params["batch_size"] = (
            params.batch_size * max(num_gpus, 1) if "batch_size" in params else 32
        )  # custom default
        ...
        if "collate_fn" in params:
            ...
            collate_fn = eval(
                f"{collate_fn_dict.DIR}" + "." + f"{collate_fn_dict.NAME}"
            )
        ...
        if (
            "NUM_ITERATIONS_PER_EPOCH" in dl_dict
        ):  ### uses custom DataLoaderForIterations class
            exec("from .dataloaders_custom import DataLoaderForIterations")
