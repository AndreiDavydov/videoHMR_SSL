...
def get_template():
    template_yaml = "configs/seq_optim/seq_optim_template.yaml"
    with open(template_yaml) as file:
        dict_file = yaml.full_load(file)
    return dict_file


def generate_config_files(weights):
    exp_names = []
    exp_paths = []
    os.makedirs("configs/seq_optim_grid_pose_shape/", exist_ok=True)
    for shape_weight in weights:
        for pose_weight in weights:
            ...
            template_dict["LOSSES"]["shape_smooth"]["WEIGHT"] = shape_weight
            template_dict["LOSSES"]["j3d_smooth"]["WEIGHT"] = pose_weight

            new_yaml = f"configs/seq_optim_grid_pose_shape/{exp_name}.yaml"
...
def write_fblearner_script(exp_names):
    f_path = "/tmp/seq_optim_grid_pose_shape.sh"
    with open(f_path, "w") as f:
        for exp_name in exp_names:
            ...
            cfg = '"cfg"'
            path = f'"manifold://xr_body/tree/personal/andreydavydov/configs/seq_optim_grid_pose_shape/{exp_name}.yaml"'
            proc += [str("'{" + f"{cfg}:{path}" + "}'")]
...
if __name__ == "__main__":
    ...
    exp_names = generate_config_files(weights)
