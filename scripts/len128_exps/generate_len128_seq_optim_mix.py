...
def get_template():
    template_yaml = "configs/seq_optim/seq_optim_template_len_128_flow_smooth_mix.yaml"
...
def write_fblearner_script(exp_names):
    f_path = "/tmp/seq_optim_flow2d_smooth_mix__len_128.sh"
    with open(f_path, "w") as f:
        for exp_name in exp_names:
            ...
            proc += [str("'{" + f"{cfg}:{path}" + "}'")]
            proc += ["--name", f"len_128__seq_{exp_name}_flow_smooth_mix"]
            proc = " ".join(proc)
